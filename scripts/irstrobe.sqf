...
Player_IR_Mark_Sides = 1; //If set to one on radar Units will come up as friendly when using IR strobe if set to 0 they come up as empty on radar
...
LALA_fnc_Strobe_addActions = {
	...
	IRstrobeOn = player addAction ["<t color='#7CFC00'>IR Strobe On</t>", {(_this select 0) spawn LALA_fnc_IRStrobeOn;},"",-99,false,true,"",'typeName (_target getVariable "strobeplayer") != "OBJECT" && (_target == _this) && (daytime > 18.7 OR daytime < 5.3) &&  if (IR_Item_Restric == 1) then { if (Player_IR_SF == 1) then { !(headgear _this in HelmetsM) } else { !(vest _this in VestsM) }; } else { if (Player_IR_SF == 1 ) then { (headgear _this != "") } else { (vest _this != "") } };'];
	IRstrobeOff = player addAction ["<t color='#7CFC00'>IR Strobe Off</t>", {(_this select 0) spawn LALA_fnc_IRStrobeOff;},"",-99,false,true,"",'typeName (_target getVariable "strobeplayer") == "OBJECT" && ( _target == _this)'];
	AttachIRAction = player addAction ["<t color='#7CFC00'>Attach IR Strobe</t>", {(_this select 0) spawn LALA_fnc_AttachIR;},"",-99,false,true,"",'(("B_IR_Grenade" in magazines _this) OR ("O_IR_Grenade" in magazines _this) OR ("I_IR_Grenade" in magazines _this)) && typeName (nearestvehicle) == "OBJECT" && (_target == _this) && (nearestvehicle getVariable "strobes") isEqualTo false && [_target] call LALA_fnc_Refresh'];
	TakeIRAction = player addAction ["<t color='#7CFC00'>Take IR Strobe</t>", {(_this select 0) spawn LALA_fnc_TakeIR;},"",-99,false,true,"",'typeName (nearestvehicle) == "OBJECT" && typeName (nearestvehicle getVariable "strobes") == "OBJECT" && (_target == _this)'];
	player addAction ["<t color='#7CFC00'>UGV Strobe On</t>", {(playeruav) spawn LALA_fnc_CreateIR;},"",-99,false,true,"",'typeName (playeruav getVariable "strobe") != "OBJECT" && (daytime > 18.7 OR daytime < 5.3) && playeruav isKindOf "LandVehicle" && (_target == _this) && [_target] call LALA_fnc_UavRefresh']; //UGV or any other land typed Uav
	player addAction ["<t color='#7CFC00'>UAV Strobe On</t>", {(playeruav) spawn LALA_fnc_CreateIRAir;},"",-99,false,true,"",'typeName (playeruav getVariable "strobe") != "OBJECT" && (daytime > 18.7 OR daytime < 5.3) && playeruav isKindOf "Air" && (_target == _this) && [_target] call LALA_fnc_UavRefresh']; //UAV or any other Air type UAV
	player addAction ["<t color='#7CFC00'>UGV Strobe Off</t>", {(playeruav) spawn LALA_fnc_DeleteIR;},"",-99,false,true,"",'typeName (playeruav getVariable "strobe") == "OBJECT" && playeruav isKindOf "LandVehicle" && (_target == _this) && [_target] call LALA_fnc_UavRefresh'];
	player addAction ["<t color='#7CFC00'>UAV Strobe Off</t>", {(playeruav) spawn LALA_fnc_DeleteIRAir;},"",-99,false,true,"",'typeName (playeruav getVariable "strobe") == "OBJECT" && playeruav isKindOf "Air" && (_target == _this) && [_target] call LALA_fnc_UavRefresh'];
};
...
LALA_fnc_Refresh = {
	...
	attachirname = format ["<t color='#7CFC00'>Attach IR Strobe to %1</t>",getText (configFile >> "cfgVehicles" >> typeOf nearestvehicle >> "displayName")];
	takeirname = format ["<t color='#7CFC00'>Take IR Strobe from %1</t>",getText (configFile >> "cfgVehicles" >> typeOf nearestvehicle >> "displayName")];
...
if (Player_IR_SF == 1) then {
	player setUserActionText [IRstrobeOn,"<t color='#7CFC00'>MS-2000 On</t>"]; 
	player setUserActionText [IRstrobeOff,"<t color='#7CFC00'>MS-2000 Off</t>"];
} else {
	player setUserActionText [IRstrobeOn,"<t color='#7CFC00'>IR Strobe On</t>"]; 
	player setUserActionText [IRstrobeOff,"<t color='#7CFC00'>IR Strobe Off</t>"];
};