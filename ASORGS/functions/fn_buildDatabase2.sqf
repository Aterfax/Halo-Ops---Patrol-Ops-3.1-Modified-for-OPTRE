...
_magazines = [];
{
	...
	if(_classname in _cfgexplosives) then {
		_explosives pushBack [DB_Explosives, _classname, _displayName, _picture, [], count _explosives, _mass];
	} else {
		if(_classname in _cfgthrowable) then {
			_throwable pushBack [DB_Throwable, _classname, _displayName, _picture, [], count _throwable, _mass];
		} else {
			_magazines pushBAck [DB_Magazines,_classname, _displayName, _picture, [], count _magazines, _mass];
		};
	};
...
{	
	...
	if(_classname in ASORGS_medical) then {
		_medical pushBack [DB_Medical, _classname,_displayName, _picture, [], count _medical, _mass]; 
		_added = true;
	};
	if(_classname in ASORGS_forceMisc) then {
		_items pushBack [DB_Misc, _classname,_displayName, _picture, [], count _items, _mass]; 
		_added = true;
	};
	if(!_added) then {
		switch (_type) do {
			case IT_HANDGUN: {
				if(!(_displayName in _handgunnames)) then
				{
					...
					_handguns pushBack [DB_Handguns, _classname,_displayName, _picture, _magindexs, count _handguns, _wpnscopes, _wpnrail, _wpnsuppressors];
				};
			};
			case IT_RIFLE: {
				if(!(_displayName in _riflenames)) then
				{
					...
					_rifles pushBack[DB_Rifles, _classname,_displayName, _picture, _magindexs, count _rifles, _wpnscopes, _wpnrail, _wpnsuppressors];
				};
			};
			case IT_LAUNCHER: {
				if(!(_displayName in _launchernames)) then
				{
					...
					_launchers pushBack[DB_Launchers, _classname,_displayName, _picture, _magindexs, count _launchers, _wpnscopes, _wpnrail, _wpnsuppressors];
				};
			};
			case IT_BINOCULAR: {
				...
				if(_infotype == 616) then {
					_nightvision pushBack [DB_Nightvision,_classname, _displayName, _picture, _magindexs, count _nightvision];
				} else {
					_binoculars pushBack [DB_Binoculars, _classname,_displayName, _picture, _magindexs, count _binoculars];
				};
			};
			case IT_ITEM: {
				switch(_infotype) do {
					case IIT_SUPPRESSOR : {
						_suppressors pushBack [DB_Suppressors, _classname,_displayName, _picture, [], count _suppressors]; 
						_allattachments pushBack [DB_Attachments, _classname, _displayName, _picture, [], count _allattachments, _mass];
					};
					case IIT_SCOPE : {
						_scopes pushBack [DB_Scopes, _classname,_displayName, _picture, [], count _scopes];
						_allattachments pushBack [DB_Attachments, _classname, _displayName, _picture, [], count _allattachments, _mass];
					};
					case IIT_RAIL : { _rails pushBack [DB_Rail, _classname,_displayName, _picture, [], count _rails]; 
							_allattachments pushBack [DB_Attachments, _classname, _displayName, _picture, [], count _allattachments, _mass];
					};
					case IIT_HEADGEAR : { _headgear pushBack [DB_Headgear, _classname,_displayName, _picture, [], count _headgear]; };
					case IIT_UNIFORM : { 
					
							_uniforms pushBack [DB_Uniforms, _classname, _displayName, _picture,_capacity, count _uniforms, _side]; 

					};
					case IIT_VEST : { _vests pushBack [DB_Vest, _classname,_displayName, _picture, _capacity, count _vests]; };
					default { 
						_simulation = tolower getText (_cfg >> "simulation");
						switch(_simulation) do {
							case "itemmap": { _maps pushBack[DB_Maps, _classname,_displayName, _picture, [], count _maps, _mass]};
							case "itemgps": { _gpss pushBack[DB_GPS, _classname,_displayName, _picture, [], count _gpss, _mass]};
							case "itemradio": { 
								_add = true;
								...
								if(_add) then {
									_radios pushBack [DB_Radios, _classname,_displayName, _picture, [], count _radios, _mass];
									_items pushBack [DB_Misc, _classname,_displayName, _picture, [], count _items, _mass];
								};
							};
							case "itemcompass": { _compass pushBack [DB_Compass, _classname,_displayName, _picture, [], count _compass, _mass]};
							case "itemwatch": { _watches pushBack [DB_Watchs, _classname,_displayName, _picture, [], count _watches, _mass]};
							default {_items pushBack [DB_Misc, _classname,_displayName, _picture, [], count _items, _mass]; };
						};
					};
				};
			};
			default {
				if(!(_displayName in _weaponnames)) then
				{
					...
					_weapons pushBack [DB_Weapons, _classname,_displayName, _picture, [], count _items];
				};
			};
		};
	};
...

{
	...
	_backpacks pushBack [DB_Backpacks, _classname, _displayName, _picture, _capacity, count _backpacks];
...
_goggles = [];
{
	...
	_scope = getNumber(_cfg >> "scope");
		...
		_goggles pushBack [DB_Goggles, _classname, _displayName, _picture, [], count _goggles];

} forEach (_allGoggleClasses);

_allInsigniaClasses = configProperties[(configFile >> "CfgUnitInsignia"), 
"isClass(_x) && {(getText(_x >> 'picture') != '') && (getText(_x >> 'displayName') != '') && {[configName(_x), true] call ASORGS_fnc_IsAllowed} }", 
true];
_insignia = [];
{	
	_cfg = _x;
	...
	if((_picture != "") && (_displayName != "")) then {
		_insignia pushBack [DB_Insignia, _classname, _displayName, _picture, [], count _insignia];
	};
...
ASORGS_DBIndexs = [];
{
	_newindex = [];
	{
		_newindex pushBack (_x select DBF_Class);
	} foreach _x;
